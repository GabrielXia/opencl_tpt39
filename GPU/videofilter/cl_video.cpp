...
{	
    ...
    while (true) {
		...
		cout << "CAMERA" << cameraFrame.size() << endl;
        Mat grayframe,edge_x,edge_y,edge;
    	cvtColor(cameraFrame, grayframe, CV_BGR2GRAY);
		//padding
		Mat padding;
		copyMakeBorder(grayframe, padding, 1, 1, 1, 1, BORDER_REPLICATE);
		...
		//memcpy(temp.data, array, padding.rows*padding.cols*sizeof(float));
		array = (float* )temp.data;

		// resize
		int blur_size = 3;
		int row_s = blur_size * blur_size;
		int wide = (int) filterframe.cols;
		int height = (int) filterframe.rows;
		...
		// get gaussianblur
//		Mat filter_g_mat = getGaussianKernel2d(blur_size, blur_size, 50, 50);
//		float *filter_g=(float*)filter_g_mat.data;
		//printf("filter: %f\n", filter_g[4]);
		float filter_g[9] = {-1, 0, 1, -1, 0, 1, -1, 0, 1};
    	...
    	status = clEnqueueWriteBuffer(queue, input_a_buf, CL_FALSE,
        	0, wide*height*row_s* sizeof(float), mat2, 0, NULL, &write_event[0]);
    	checkError(status, "Failed to transfer input A");

    	status = clEnqueueWriteBuffer(queue, input_b_buf, CL_FALSE,
        	0, row_s* sizeof(float), filter_g, 0, NULL, &write_event[1]);
