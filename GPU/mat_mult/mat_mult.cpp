...
float *ref_output=(float *) malloc(sizeof(float)*size);
...
int status;
	...
	for(unsigned x = 0; x < N; ++x) {
	    for(unsigned y = 0; y < N; ++y) {
    		...
    		for (unsigned k=0; k<N; ++k) {
        		float a = input_a[k + N * y];
        		float b = input_b[k*N +x];
        		value += a * b;
    		}
			ref_output[x + N * y] = value;		
		}  
	      //printf("ref %f\n",ref_output[j]);
	    }
	time (&end);
    ...
    time (&start);
     ...
     unsigned char **opencl_program=read_file("mat_mult.cl");
 ...
 // Input buffers.
    ...
    status = clEnqueueWriteBuffer(queue, input_a_buf, CL_FALSE,
        0, size* sizeof(float), input_a, 0, NULL, &write_event[0]);
    checkError(status, "Failed to transfer input A");

    status = clEnqueueWriteBuffer(queue, input_b_buf, CL_FALSE,
        0, size* sizeof(float), input_b, 0, NULL, &write_event[1]);
	...
	for (unsigned t=0; t<1000000000;++t);
